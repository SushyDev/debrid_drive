defmodule SyncEngine.Services.TorrentVerifier do
  ...
  def verify_torrent(%Torrent{} = torrent) do
    ...
    if torrent.inode_id do
      case VFS.get_node(torrent.inode_id) do
        {:ok, _node} ->
  ...
  defp verify_torrent_files(%Torrent{} = torrent) do
    ...
    if remaining_files == 0 and length(files) > 0 do
      Logger.info("Torrent #{torrent.filename} has no remaining files, removing torrent and directory")
  ...
  defp verify_torrent_file(%TorrentFile{} = file) do
    if file.inode_id do
      ...
      case VFS.get_node(file.inode_id) do
        {:ok, _node} ->
          # File exists
        ...
        {:error, :not_found} ->
          ...
          Logger.debug("File node #{file.inode_id} missing, removing torrent file record")
